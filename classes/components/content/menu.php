<?php
	...
	class ContentMenu {
		 ...
		 * @return array|string
		 */
		public function generateMenuResult($menuItems, $templates, $rootPageId, $rootPageURI, $URISuffix, iUmiHierarchy $umiHierarchy, $activeElementsIds, $currentDepth = 0, $maxDepth = 1, $isXslt, $showHasChildren) {
			...
			foreach ($rootMenuItems as $rootMenuItem) {
				...
				if ($isXslt) {
					if (is_array($subMenu) && isset($subMenu['items'], $subMenu['items']['nodes:item']) && sizeof($subMenu['items']['nodes:item'])) {
