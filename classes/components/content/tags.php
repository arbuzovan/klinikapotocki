<?php
	...
	class ContentTags {
		...
		public function tagsAccountCloud($s_template = "tags", $b_curr_user = false, $i_per_page = -1, $b_ignore_paging = true) {
			return $this->tags_mk_cloud(null, $s_template, $i_per_page, $b_ignore_paging, false, ($b_curr_user ? UmiCms\Service::Auth()->getUserId() : []));
		}
		...
		public function tagsAccountEfficiencyCloud($s_template = "tags", $b_curr_user = false, $i_per_page = -1, $b_ignore_paging = true) {
			return $this->tags_mk_eff_cloud(null, $s_template, $i_per_page, $b_ignore_paging, ($b_curr_user ? UmiCms\Service::Auth()->getUserId() : []));
		}
		...
		public function tagsDomainCloud($s_template = "tags", $b_curr_user = false, $i_per_page = -1, $b_ignore_paging = true) {
			return $this->tags_mk_cloud(cmsController::getInstance()->getCurrentDomain()->getId(), $s_template, $i_per_page, $b_ignore_paging, false, ($b_curr_user ? UmiCms\Service::Auth()->getUserId() : []));
		}
		...
		public function tagsDomainEfficiencyCloud($s_template = "tags", $b_curr_user = false, $i_per_page = -1, $b_ignore_paging = true) {
			return $this->tags_mk_eff_cloud(cmsController::getInstance()->getCurrentDomain()->getId(), $s_template, $i_per_page, $b_ignore_paging, ($b_curr_user ? UmiCms\Service::Auth()->getUserId() : []));
		}
		...
		public function tagsDomainUsageCloud($s_template = "tags", $i_per_page = -1, $b_ignore_paging = true) {
			return $this->tags_mk_cloud(cmsController::getInstance()->getCurrentDomain()->getId(), $s_template, $i_per_page, $b_ignore_paging, false, []);
		}
		 ...
		 * @return mixed
		 */
		private function pages_mklist_by_tags($tags, $currentDomainOnly = null, $template = "tags", $perPage = false, $ignorePaging = false, $baseTypes = '') {
			...
			if (strlen($baseTypes)) {
				...
				foreach ($arrBaseTypes as $s_next_type) {
					...
					if (count($arrNextType) === 2) {
						$hierarchyType = umiHierarchyTypesCollection::getInstance()->getTypeByName($arrNextType[0], $arrNextType[1]);
			...
			foreach ($result as $element) {
				...
				if ($publishTime = $element->getValue('publish_time')) {
		 ...
		 */
		private function tags_mk_cloud($i_domain_id = null, $s_template = "tags", $i_per_page = -1, $b_ignore_paging = true, $b_by_usage = false, $arr_users = []) {
			// init and context :
		 ...
		 * @throws coreException
		 */
		private function tags_mk_eff_cloud($i_domain_id = null, $s_template = "tags", $i_per_page = -1, $b_ignore_paging = true, $arr_users = []) {
			if (!$arr_users || intval($arr_users) === -1 || strval($arr_users) === getLabel('page_status_all')) {
				$arr_users = [];
			}
			if (is_int($arr_users)) {
